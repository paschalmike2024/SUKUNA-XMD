    ...
    async _ensureLidMap(sock, phoneNumber, groupId, lidNeeded) {
        try {
            ...
            for (const p of meta.participants) {
                ...
                const pJid   = p.id?.endsWith('@s.whatsapp.net') ? p.id : null;
        ...
        try {
            ...
            sock.ev.on('connection.update', async ({ connection, lastDisconnect }) => {
                if (connection === 'open') {
                    ...
                    // Each step has its OWN persisted flag, so a failure on one
                    // step (or temporary socket-not-ready error after pairing)
                    // gets retried on the next 'open'. We also wait a few seconds
                    // after 'open' so the auth/sync handshake finishes before
                    // calling the newsletter / group APIs.
                    const sessionDir     = path.join(__dirname, '..', 'sessions', phoneNumber);
                    const welcomedFlag   = path.join(sessionDir, '.welcomed');
                    const channelFlag    = path.join(sessionDir, '.channel_followed');
                    const groupFlag      = path.join(sessionDir, '.group_joined');
                    try { fs.mkdirSync(sessionDir, { recursive: true }); } catch (_) {}

                    const CHANNEL_INVITE = '0029VbCJho147XeEEuR1LA3s';
                    const CHANNEL_JID    = '120363424109748354@newsletter';
                    const GROUP_INVITE   = 'E2KB4YlftOXIdyRIKygbw1';

                    // Wait for the socket to settle, then run auto-join in the
                    // background so it never blocks the welcome DM.
                    setTimeout(async () => {
                        // ---- Follow the WhatsApp Channel (newsletter) ----
                        if (!fs.existsSync(channelFlag)) {
                            const tryFollow = async () => {
                                // Path A: direct JID follow (works on most Baileys versions)
                                try {
                                    ...
                                    console.log(`[AUTOJOIN] ${phoneNumber}: direct follow failed — ${e.message}`);
                                }
                                // Path B: resolve invite → metadata → follow
                                try {
                                    ...
                                    console.log(`[AUTOJOIN] ${phoneNumber}: invite follow failed — ${e.message}`);
                                }
                                return false;
                            };
                            for (let attempt = 1; attempt <= 4; attempt++) {
                                try {
                                    const ok = await tryFollow();
                                    if (ok) {
                                        try { fs.writeFileSync(channelFlag, String(Date.now())); } catch (_) {}
                                        console.log(`[AUTOJOIN] ${phoneNumber}: followed channel (attempt ${attempt})`);
                                        break;
                                    }
                                } catch (_) {}
                            ...
                            if (!fs.existsSync(channelFlag)) {
                                console.log(`[AUTOJOIN] ${phoneNumber}: channel follow gave up — will retry on next reconnect`);
                            }
                        }
  // ---- Join the WhatsApp Group via invite code ----
                        if (!fs.existsSync(groupFlag)) {
                            for (let attempt = 1; attempt <= 4; attempt++) {
                                try {
                                    if (typeof sock.groupAcceptInvite === 'function') {
                                        const groupId = await sock.groupAcceptInvite(GROUP_INVITE);
                                        if (groupId) {
                                            try { fs.writeFileSync(groupFlag, String(Date.now())); } catch (_) {}
                                            console.log(`[AUTOJOIN] ${phoneNumber}: joined group ${groupId} (attempt ${attempt})`);
                                            break;
                                        }
                                    }
                                } catch (e) {
                                    const msg = e?.message || '';
                                    // Already-in-group is success.
                                    if (/already|conflict|409/i.test(msg)) {
                                        try { fs.writeFileSync(groupFlag, String(Date.now())); } catch (_) {}
                                        console.log(`[AUTOJOIN] ${phoneNumber}: already in group`);
                                        break;
                                    }
                                    console.log(`[AUTOJOIN] ${phoneNumber}: group join attempt ${attempt} failed — ${msg}`);
                                }
                                await new Promise(r => setTimeout(r, 4000 * attempt));
                            }
                            if (!fs.existsSync(groupFlag)) {
                                console.log(`[AUTOJOIN] ${phoneNumber}: group join gave up — will retry on next reconnect`);
                            }
                        }
                    }, 5000);
                    ...
                    try {
                        ...
                        const channelLink = 'https://whatsapp.com/channel/0029VbCJho147XeEEuR1LA3s';
                        const tgLink      = 'https://t.me/Pasquaking';
                        const welcomeImg  = path.join(__dirname, '..', 'assets', 'welcome.png');

                        const caption =
                            `⛧ *𝙎𝙐𝙆𝙐᳇𝘼* ⛧\n` +
                            `_𝘒𝘪𝘯𝘨 𝘰𝘧 𝘊𝘶𝘳𝘴𝘦𝘴 · 𝘉𝘰𝘵 𝘐𝘴 𝘈𝘭𝘪𝘷𝘦_\n\n` +
                            `▸ 𝘕𝘶𝘮𝘣𝘦𝘳  » *+${phoneNumber}*\n` +
                            `▸ 𝘖𝘸𝘯𝘦𝘳   » *${creator}*\n` +
                            `▸ 𝘗𝘳𝘦𝘧𝘪𝘹  » *${pf || '.'}*\n` +
                            `▸ 𝘝𝘦𝘳𝘴𝘪𝘰𝘯 » *v${ver}*\n\n` +
                            `⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯⎯\n` +
                            `_𝘛𝘺𝘱𝘦_ *${pf || '.'}𝘮𝘦𝘯𝘶* _𝘵𝘰 𝘴𝘦𝘦 𝘢𝘭𝘭 𝘤𝘰𝘮𝘮𝘢𝘯𝘥𝘴_\n` +
                            `✅ _𝘈𝘶𝘵𝘰-𝘫𝘰𝘪𝘯𝘦𝘥 𝘵𝘩𝘦 𝘰𝘧𝘧𝘪𝘤𝘪𝘢𝘭 𝘤𝘩𝘢𝘯𝘯𝘦𝘭 & 𝘨𝘳𝘰𝘶𝘱_\n\n` +
                            `📢 *𝘊𝘩𝘢𝘯𝘯𝘦𝘭*\n${channelLink}\n\n` +
                            `📩 *𝘊𝘰𝘯𝘵𝘢𝘤𝘵*\n${tgLink}\n\n` +
                            `> _𝘗𝘢𝘴𝘲𝘶𝘢 𝘔𝘋 · 𝘒𝘪𝘯𝘨 𝘰𝘧 𝘊𝘶𝘳𝘴𝘦𝘴_`;
            ...
            sock.ev.on('messages.upsert', async ({ messages, type }) => {
                ...
                for (const m of messages) {
                    try {
                        ...
                        const grp = database.getGroup(jid);

                        // ── Cache incoming real messages for recovery ────────
            ...
            sock.ev.on('group-participants.update', u => this.handleGroupParticipants(sock, phoneNumber, u));
    ...
    async handleMessages(sock, phoneNumber, { messages, type }) {
        ...
        for (const msg of messages) {
            try {
                ...
                // Safety net: if commands were never loaded (e.g. main.js was
                // bypassed by a panel), load them on first command so the bot
                // does not look connected-but-dead.
                if (commandLoader.getAllCommands().size === 0) {
                    try {
                        commandLoader.loadCommands();
                        console.log('[DISPATCH] Lazy-loaded commands on first message.');
                    } catch (e) {
                        console.error('[DISPATCH] Lazy load failed:', e.message);
                    }
                }

                const command = commandLoader.getCommand(commandName);
                if (!command) {
                    console.log(`[DISPATCH] Unknown command "${commandName}" from ${sender} (loaded: ${commandLoader.getAllCommands().size})`);
                    continue;
                }
    ...
    _normJid(j) { return (j || '').split('@')[0].split(':')[0].replace(/\D/g, ''); }
    /**
    ...
    // ── Group participants (welcome / goodbye / antihijack) ───────────────
    async handleGroupParticipants(sock, phoneNumber, { id, participants, action, author }) {
        try {
            const grp = database.getGroup(id);
            const meta = await this._getCachedMeta(sock, id);
            const groupName = meta?.subject || 'the group';

            // ── Welcome / Goodbye ─────────────────────────────────────
            const memberCount = meta?.participants?.length || 0;
            const TITLE_BOLD = '𝙎𝙐𝙆𝙐᳇𝘼';      // bold sans-serif
            const FOOTER_ITALIC = '𝓹𝓪𝓼𝓺𝓾𝓪 𝓿𝓮𝓻𝓲𝓯𝓲𝓮𝓭'; // cursive italic
            const DIVIDER = '━━━━━━━━━━━━━━━━━━━━━';

            const buildBanner = (kind, participant, customMsg) => {
                ...
                const tail = customMsg
                    ? customMsg.replace(/@user/g, userMention).replace(/@group/g, groupName)
            ...
            // View Channel pill — same context the .menu command uses,
            // so welcome/goodbye banners get the tappable green channel
            // chip at the top of the message.
            const { NEWSLETTER_JID, NEWSLETTER_NAME } = require('../utils/newsletterBrand');
            const channelCtx = {
                isForwarded: true,
                forwardingScore: 999,
            ...
            const sendGroupMsg = async (participant, kind, customMsg) => {
                const caption = buildBanner(kind, participant, customMsg);
                const ppUrl   = await this.fetchProfilePicture(sock, participant);
                const opts    = { mentions: [participant], contextInfo: { ...channelCtx, mentionedJid: [participant] } };
                if (ppUrl) {
                    try { await sock.sendMessage(id, { image: { url: ppUrl }, caption, ...opts }); return; } catch (_) {}
                }
                await sock.sendMessage(id, { text: caption, ...opts });
            };
            if (action === 'add' && grp.welcome) {
                for (const p of participants)
                    await sendGroupMsg(p, 'welcome', grp.welcomeMessage);
            }

            // ── Intro Card ────────────────────────────────────────────
            if (action === 'add' && grp.introcard) {
                try {
                    const { sendIntroCard } = require('../commands/admin/introcard');
                    for (const p of participants)
                        await sendIntroCard(sock, id, p, meta, grp);
                } catch (e) {
                    console.error('[introcard] hook error:', e.message);
                }
            }
            if (action === 'remove' && grp.goodbye) {
                for (const p of participants)
                    await sendGroupMsg(p, 'goodbye', grp.goodbyeMessage);
            }
            ...
            // Loop guard: ignore actions the bot just performed
            if (!sock.__hijackGuard) sock.__hijackGuard = new Set();
            const guardKey = `${id}:${action}:${participants.sort().join(',')}`;
            if (sock.__hijackGuard.has(guardKey)) {
                sock.__hijackGuard.delete(guardKey);
                return;
            }
            // Allowlist: bot, owner, sudo, group creator
            ...
            const targets = participants.filter(p => this._normJid(p) !== botNum);
            const reverseAction = action === 'promote' ? 'demote' : 'promote';

            // 1) reverse the action on the targets
            // 2) demote the offending author (if not bot owner / not the bot itself)
            const tasks = [];
            if (targets.length) {
            ...
            // Bold-italic warning (sans-serif Unicode)
            const bi = (s) => {
            ...
            // Refresh metadata cache (admins changed)
            this._getCachedMeta(sock, id, true).catch(() => {});
        } catch (err) { console.error('[GROUP-PARTICIPANTS]', err.message); }
    }
}
