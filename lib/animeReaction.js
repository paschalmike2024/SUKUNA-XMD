 ...
 *   3. https://apis.prexzyvilla.site/anime/<name> ← original (often 5xx)
...
async function resolveGifUrl(name) {
    ...
    // 3) prexzyvilla (often 500 — last resort)
    const j = await fetchJson(`https://apis.prexzyvilla.site/anime/${name}`);
    const u = pickUrl(j);
    if (u) return u;
    return null;
}
