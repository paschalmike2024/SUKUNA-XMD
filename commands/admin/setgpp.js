...
module.exports = {
    ...
    async execute({ sock, msg, from, reply, isGroup, isAdmin, isBotAdmin }) {
        if (!isGroup)    return reply('❌ This command is for groups only.');
        if (!isAdmin)    return reply('❌ Only admins can change the group picture.');
        if (!isBotAdmin) return reply('❌ I need to be an admin to change the group picture.');
        ...
        try {
            ...
            reply('❌ Failed to update group picture. Make sure I am an admin.');
        }
    },
};
